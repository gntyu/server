...
const generatorRule = function(rule) {
  ...
  rule.forEach(item => {
    let type = '';
    const name = item[0];
    const dataType = item[1];
    const length = item[2];
    let max = 0;
    const required = item[3];
    const allowEmpty = item[4];
    ...
    // 允许空字符串
    if (dataType === 'varchar') {
      res[name] = Object.assign({ allowEmpty }, res[name]);
    }
  });
  console.log(res);
  return res;
...
const convertImgCmd = function(path, thumbImgPath, size) {
  return new Promise((resolve, reject) => {
    const convertImgCmdStr = `convert ${path} -resize ${size} ${thumbImgPath}`;
