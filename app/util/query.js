...
module.exports.resultFormat = function(queryData) {
  ...
  for (const item in queryData) {
    const table = item.split('_')[0];
    const key = item.split('_')[1];
    const value = queryData[item];
    const obj = {
      [key]: value,
    };
    list.push(table);
...
const getOrderQuery = function(orders, entityAlias) {
  ...
  for (let i = 0; i < orders.length; i++) {
    const name = orders[i][0];
    const order = orders[i][1] || 'desc';
...
const pagination = function(pageAt, pageLimit) {
  ...
  const offset = (pageAt - 1) * pageLimit;
...
const Columnmaker = function(entity) {
  ...
  for (let x = 0; x < entity.relatedEntities.length; x++) {
    const relatedEntity = entities[entity.relatedEntities[x].entity];
...
const joinMaker = function(mainEntityName, relatedEntities) {
  ...
  relatedEntities.forEach(item => {
    const mainEntity = entities[mainEntityName];
    const subEntity = entities[item.entity];
    let mainKey = '';
    let subKey = '';
