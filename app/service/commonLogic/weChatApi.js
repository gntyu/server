...
const getPrePayId = function(appId, orderId, totalFee, name, ipAddress, url, openid, attach) {
  return new Promise((resolve, reject) => {
    const https = require('https');
    const nonce_str = Tools.randomString(16);
    let tempStr = `appid=${appId}&attach=${attach}&body=${name}&device_info=WEB&fee_type=CNY&mch_id=${setting.wechatInfo.mch_id}&nonce_str=${nonce_str}&notify_url=${url}&openid=${openid}&out_trade_no=${orderId}&sign_type=MD5&spbill_create_ip=${ipAddress}&total_fee=${totalFee}&trade_type=JSAPI`;
    tempStr = tempStr + `&key=${setting.wechatInfo.payPassWord}`;
    const sign = CryptoJS.MD5(tempStr).toString().toUpperCase();
    const postData = {
      appid: appId,
      attach,
      mch_id: setting.wechatInfo.mch_id,
    ...
    const xml = builder.buildObject(postData);

    const contentStr = xml;
    const contentLen = Buffer.byteLength(xml, 'utf8');
    const options = {
    ...
    const _req = https.request(options, function(res) {
      res.setEncoding('utf8');
      res.on('data', function(chunk) {
        // xml转json
...
module.exports = app => {
  return class weChatApi extends app.Service {
    ...
    * getToken() {
      try {
        let localToken = yield getLocalToken();
        let token = '';
    ...
    * getTicket() {
      try {
        let localTicket = yield getLocalTicket();
        let ticket = '';
    ...
    * getUserInfo(openid, type = 'snsapi_base', data = null) {
      try {
        ...
        let userInfoLocal = yield app.mysql.query(Tools.generatorQuery('userinfo_extend', 1, { openid }, null, 1, 'query'));
        ...
        // 再次获取用户信息
        userInfoLocal = yield app.mysql.query(Tools.generatorQuery('userinfo_extend', 1, { openid }, null, 1, 'query'));
        return Response.success({
    ...
    * getPrePayId(data) {
      try {
        const _orderId = data.orderId.split('-').join('');
        const orderId = data.orderId;
        const attach = data.orderId;
        const name = data.productName;
        const ipAddress = setting.wechatInfo.ipAddress;
        const url = setting.wechatInfo.payCallbackUrl;
        ...
        let prePayId = yield getPrePayId(appId, _orderId, payInfo[0].totalPrice, name, ipAddress, url, openid, attach);
        if (prePayId.result_code == 'FAIL') {
          // 因为订单订单id重复
          const newOrderId = uuid();
          const _newOrderId = newOrderId.split('-').join('');
          // 更新所有关于订单类表的orderId
    ...
    * getPayNotify(data) {
      try {
        ...
        // 去掉-的uuid转换
        let out_trade_no = notifyInfo.out_trade_no;
        ...
        if (result.length == 0) {
          ...
          try {
            ...
            // 发送短信
            const tel = yield this.app.mysql.query(`select tel from ${setting.PREFIX}userinfo_extend where openid='${orderInfo[0].openid}'`);
